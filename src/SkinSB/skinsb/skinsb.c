...
BOOL WINAPI SkinSB_Init(HWND hwnd, HBITMAP hBmp)
{
	...
	psb->hwnd = hwnd;
	psb->pfnOldProc = (WNDPROC)(LONG_PTR)SetWindowLongPtr(hwnd, 
		GWLP_WNDPROC, (LONG_PTR)SkinSB_Proc);
...
BOOL WINAPI SkinSB_Uninit(HWND hwnd)
{
	...
	SetWindowLongPtr(hwnd, GWLP_WNDPROC, (LONG_PTR)psb->pfnOldProc);
...
UINT SkinSB_GetState(LPSKINSB psb, BOOL fVert, UINT nHitItem)
{
	...
	if( (nHitItem == GET_HTITEM(psb->dwLastCode)) && (fVert == GET_HTVERT(psb->dwLastCode)) )
		fHotTracked = TRUE;
